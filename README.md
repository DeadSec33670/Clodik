...
- **админ-панель** — http://localhost:3000/admin (пароль из `ARHO_ADMIN_PASSWORD`, по умолчанию `admin`)
- здоровье API — http://localhost:3000/api/health

### Админ-панель (`/admin`)

Статистика заказов: выручка, средний чек, заказы по дням и статусам, топ товаров,
распределение клиентов по уровням лояльности, топ клиентов, таблица последних заказов
со сменой статуса (создан → оплачен → отправлен → доставлен / отменён).
Вход по паролю `ARHO_ADMIN_PASSWORD` — **обязательно смените его в проде.**
